...
def ver_propostas_disponiveis():
    ...
    for k in range(qtd):
        results_cidade_partida[k] = str(db.child("propostas/"+str(k+1)+"/cidade_partida").get(user['idToken']).val())
    for l in range(qtd):
        results_cidade_destino[l] = str(db.child("propostas/"+str(l+1)+"/cidade_destino").get(user['idToken']).val())
    for m in range(qtd):
        results_valor_proposto[m] = str(db.child("propostas/"+str(m+1)+"/valor_proposto").get(user['idToken']).val())
    
    return qtd,results_cpf,results_descricao,results_cidade_partida,results_cidade_destino,results_valor_proposto
...
def minhas_propostas(cpf):
    ...
    if(str(all_cpfs_propostas.val()) != "None"):
        result = []
        results_descricao = []
        results_cidade_partida = []
        results_cidade_destino = []
